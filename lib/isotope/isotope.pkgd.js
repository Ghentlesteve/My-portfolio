...
proto._transitionTo = function( x, y ) {
  ...
  let didNotMove = compareX === this.position.x && compareY === this.position.y;
...
proto.updateSortData = function() {
  ...
  let getSortData = this.layout.options.getSortData;
...
( function( window, factory ) {
  ...
  **/
  ...
  proto.needsVerticalResizeLayout = function() {
    // don't trigger if size did not change
    let size = getSize( this.isotope.element );
    // check that this.size and size are there
    // IE8 triggers resize on body size change, so they might not be
    let hasSizes = this.isotope.size && size;
  ...
  **/
  proto.getSegmentSize = function( segment, size ) {
    ...
    let firstItemSize = this.getFirstItemSize();
  ...
  proto.getFirstItemSize = function() {
    let firstItem = this.isotope.filteredItems[0];
...
// -------------------------- masonryDefinition -------------------------- //
  ...
  let proto = MasonryMode.prototype;
...
proto._getItemLayoutPosition = function( item ) {
  item.getSize();
  let x = ( this.isotope.size.innerWidth - item.size.outerWidth ) *
...
// -------------------------- isotopeDefinition -------------------------- //
  ...
  proto._getIsInstant = function() {
    ...
    let isInstant = isLayoutInstant !== undefined ? isLayoutInstant :
